...
```{r}
library(readxl)
mr_trash_wheel = read_excel("./data/HealthyHarborWaterWheelTotals2017-9-26.xlsx", "Mr. Trash Wheel", range = "A2:N258") %>% #import mr trash wheel datasheet from cell A2 to N258
...
precip_data_2016 = read_excel("./data/HealthyHarborWaterWheelTotals2017-9-26.xlsx", "2016 Precipitation", range = "A2:B14") %>%
  janitor::clean_names()
  
precip_data_2017 = read_excel("./data/HealthyHarborWaterWheelTotals2017-9-26.xlsx", "2017 Precipitation", range = "A2:B14") %>%
  janitor::clean_names() %>%
  filter(!is.na(total)) %>%


left_join(precip_data_2016, precip_data_2017)
